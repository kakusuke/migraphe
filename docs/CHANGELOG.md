...
- **履歴スキーマの進化・移行機構を導入（検出SQLと適用SQLの組）**
  ...
  - **機構**: リソースを**ステップの列**として記述する形式に変更。`--@apply` が適用SQL群を導入して新しいステップを開始し、その前に置く `--@check` は**省略可能**な検出SQLを宣言する。`SchemaStepParser` が `SchemaStep(label, checkSql, applySql)` に変換し、`initialize()` が順に回して1文ずつ実行する。検出SQLがあり**1行以上返せば適用済み**としてスキップ、無ければ常に適用する。
  - **検出SQLは意図的に省略可能であり、出荷する3リソースはいずれも使っていない**（レビューでの設計変更）。出荷ステップは全て「オブジェクトを作る」ものであり、`IF NOT EXISTS`（`CREATE TABLE` は MySQL/MariaDB/H2/PostgreSQL 9.1+、`CREATE INDEX` は PostgreSQL 9.5+）が既に条件付き作成を表現でき、しかも**別スキーマの同名テーブルと混同しようがない**。作成用の検出SQLは「このオブジェクトが**ここに**あるか」を答える必要があるが、汎用 JDBC リソースはそれができない: `type="jdbc"` の全 DB で共有され、H2・MySQL・PostgreSQL を貫く「現在のスキーマ」式が存在しない（`SCHEMA()` は H2 と MySQL では通るが PostgreSQL を壊す）。非修飾の `UPPER(table_name)` 比較では**同一 MySQL サーバの別データベースにある `migraphe_history` にヒットし、作成ステップがスキップされてテーブルが作られないまま INSERT が落ちる**。よって方針は「**作成は `IF NOT EXISTS` に任せ、検出は可搬な条件付き形式が無い変更のために取っておく**」— 主に `ALTER TABLE ... ADD COLUMN`（MariaDB と PostgreSQL には `IF NOT EXISTS` があるが **Oracle MySQL には無い**）。将来の検出SQLのスキーマ修飾（`conn.getCatalog()` / `getSchema()` を位置パラメータで束縛する案）は、最初の列を足す PR で決める。
  - **バージョン表を持たない理由**: 冪等性が構造的に保証され、記録されたバージョン番号と実体がずれ得ない。途中で失敗しても次回コマンドで止まった所から再開する。DDL を手で管理したい利用者は先に流しておけば `IF NOT EXISTS` か検出SQLがステップを no-op にするので、`auto-migrate` 相当の設定キーは不要（逃げ道が機構自体に内在する）。型変更ステップも出荷しない: `id VARCHAR(255)` が問題になるのはインデックスキーのときだけで、既存テーブルは上限 3072 バイトの環境で作られている。
  - **検出SQLの制約**: テーブル不在でも実行できる必要があるため `information_schema` / `pg_indexes` を見る。**検出SQLの失敗は「未適用」に丸めず伝播させる**（権限エラーや接続断を未適用と解釈すると、盲目的な DDL 実行に化ける）。
  - **並行実行**: 適用が失敗したら検出をやり直し、既に適用済みなら**飲み込む**（競合プロセスが同じステップを適用したケース）。再検出自体が失敗した場合は元の例外を suppressed 付きで投げる。検出SQLを持たないステップでは再検出が「未適用」を返して失敗が伝播するが、これは正しい（競合は `IF NOT EXISTS` が既に吸収している）。方言別ロックは持ち込まない。
  - **全コマンド共通経路**: `up` / `down` / `status` はいずれも従来から無条件で `initialize()` を呼んでおり、`status` は既にテーブルを作っていた。よってステップ適用を加えても新しい性質は生まれない。将来 `ORDER BY` が新列を参照するステップが入っても `status` が壊れない利点もある。
  - **後方互換**: ディレクティブを1つも含まないリソースは**無条件の1ステップ**として扱うので、公開コンストラクタ `JdbcHistoryRepository(env, path)` に独自 DDL を渡している外部プラグインはそのまま動く。逆に、最初のディレクティブより前に SQL があるリソースは**黙って捨てずにエラー**にする。
  - **副次的な利点**: PostgreSQL はテーブルと各インデックスを別ステップに分けた。3文を1回の `execute()` に渡していた（PG ドライバが複数文を許すので動いていた）状態が解消され、1文ずつ実行される。診断メッセージが失敗ステップを名指しでき、**手で削除したインデックスが次回実行で自己修復**する（常に実行＋`IF NOT EXISTS`）。
  - **ラベル規則の明確化（挙動変更2件）**: `--@check` / `--@apply` のどちらがラベルを宣言してもよく、宣言したほうを採る。両方が宣言した場合は**一致していなければ例外**（不変条件2）。従来は `--@check` が組を開くと `--@apply` 側のラベルを黙殺していたため、**(a) `--@check`（無名）+ `--@apply B` のラベルが `step N` から `B` に変わり**、**(b) `--@check A` + `--@apply B` は A を採って B を捨てていたのが例外になる**。パースは組を機械的に作るだけでラベルを一切見ない（`--@check` は常に新しい組を開き、`--@apply` は現在の組の空いている apply 枠を埋める。埋まっていれば新しい組を開く）。**ラベルの一意性は要求しない** — 別々の組が同じラベルを持ってよい。`label` は例外メッセージ (`Failed to apply schema step '<label>'` / `Failed to detect schema step '<label>'`) にのみ現れ、永続化も比較もされないので、一意性を強制しても得るものが無く、正しく書けているリソースを弾くだけだから。不変条件4つとラベル規則は `SchemaStepParser` のクラス Javadoc に明記した。
  - **パーサの構造整理（振る舞い不変）**: `label == null` / `hasCheck` / `inApply` の3つの状態変数を `enum Section { NONE, CHECK, APPLY }` 1つ＋`@Nullable StringBuilder check`（null ⇒ `--@check` 未出現）に畳んだ。`containsDirective` による先読み1パスと後方互換の特例分岐を両方削除し、「一度も `NONE` を出なかった＝ディレクティブが無かった」ので EOF 時に pending をそのまま無条件ステップにする形にした。ヘッダの trivia 検証は遅延（行のリストを溜めて後で検証）をやめ、`NONE` 中に非 trivia 行が来た時点で記録し `NONE` を出る瞬間に判定する。1行の解析は `record Directive(Kind, @Nullable String label)` を返す1回に集約し、「未宣言」を空文字ではなく `null` で表現するようにした（空文字のままだと素の `--@check` + 素の `--@apply` を「両方宣言あり」と誤判定してしまう）。
  - **テスト**: 31本追加、計 **1,085本** green。`SchemaStepParserTest`(22) はマーカー無しのフォールバック、`--@apply` 単独（検出SQL無し）ステップ、`--@apply` 連続時の分割、検出有り／無しの混在、ヘッダコメント無視、ラベル規則6種（check 側／apply 側／両方一致／両方不一致で例外／位置ベース既定／別ステップ間の重複許可）、不正リソース6種を固定。境界規則のうち3つ（`--@check` 連続、`--@check` の本文が空、`--@apply` の本文が空）は構造を触る前に特性テストとして固定した（3つ目は既存テストのどれからも到達していない分岐だった）。`JdbcHistoryRepositorySchemaStepsTest`(6, H2) は**テスト用リソース**で検出機構を維持検証: 検出によるスキップ／適用／**競合時の飲み込み**／再検出でも未適用なら伝播／検出SQL失敗の伝播、および出荷リソース（検出SQL無し）の冪等性。飲み込み分岐は「同じ `CREATE TABLE` を2文並べたリソース」で決定的に再現した（1文目が成功し2文目が競合プロセスと同じ失敗をする）。MariaDB 10.1 と PostgreSQL には「`initialize()` を2回呼んでもエラーにならない」冪等性テストを置き（出荷リソースが検出SQLを持たなくなったため、検出SQL検証テストから置き換え）、PG では「インデックスを落として再 initialize すると再作成される」自己修復も検証。4方言（H2 / MySQL 8.0 / MariaDB 10.1 / PostgreSQL 16）すべて green。
  - **検証の過程で見つけた自作のテスト不備**: H2 の in-memory DB に `DB_CLOSE_DELAY=-1` を付け忘れており、接続クローズでスキーマが消えるため「検出できたステップはスキップされる」テストが**誤った理由で PASS** していた。修正後、`isApplied` を常に false にする手動ミューテーションで3本が落ちることを確認して偽陽性でないことを担保した。
