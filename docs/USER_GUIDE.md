...
**Rows the task files no longer declare are the half `upgrade-history` cannot reach.** There is no definition
...
- Confirm the JitPack build of `v0.7.0` finished successfully at <https://jitpack.io/#kakusuke/migraphe>
...
Plugin JARs and the Gradle plugin are currently distributed via JitPack at `com.github.kakusuke.migraphe:<module>:v0.7.0`. Maven Central publication is planned; the groupId will switch to `io.github.kakusuke.migraphe` at that point.
