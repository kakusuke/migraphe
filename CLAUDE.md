...
**Current Phase**: 22 (JitPack distribution) - COMPLETE; latest work: made "latest record" deterministic by minting time-ordered UUIDv7 ids (ordering is `executed_at DESC, id DESC`) and renamed the history column `environment_id` to `target_id` through a detection-guarded migration step (Session 71)
**Tests**: 1,109, 100% passing
...
- **"Latest record" is now deterministic.** `wasExecuted()` / `findLatestRecord()` ordered by `executed_at DESC LIMIT 1`, undefined when a node's records share a timestamp — and on MariaDB they routinely do: Connector/J reads the server's `5.5.5-10.1.48-MariaDB` version string, decides it predates MySQL 5.6.4 and **truncates fractional seconds client-side** (measured; unavoidable via `sendFractionalSeconds` / `useServerPrepStmts` in any combination, while the MariaDB driver keeps them — so the driver alone is the cause). A `down` immediately followed by an `up` then tied, and the storage engine picked the winner: a rolled-back node could be silently reported as applied.
- **Fixed by minting time-ordered ids rather than adding a column.** `ExecutionRecord`'s four factories now generate **UUIDv7** (RFC 9562) instead of UUIDv4, so the already-present `id` primary key sorts in creation order. No schema change, no per-dialect auto-increment syntax (which the generic JDBC resource could not express portably), and the InnoDB primary key becomes append-ordered as a bonus. Ordering is **`executed_at DESC, id DESC`** — keeping `executed_at` primary leaves rows written by older versions, whose ids are random UUIDv4, ordered exactly as before; only ties consult the id. `executedNodes()` switches from `MAX(executed_at)` matching to a correlated scalar subquery with the same rule, which also drops the `DISTINCT` it needed.
- `RecordIds` is **package-private** in `migraphe-api`: the canonical constructor still accepts any string, so the format stays an implementation detail. Monotonic within a millisecond through the 12-bit `rand_a` counter (RFC 9562 §6.2 method 1), borrowing the next millisecond on overflow; a **backwards clock never rewinds** — the timestamp is kept and the counter advances. The 62 random bits still carry cross-process uniqueness.
- **`environment_id` → `target_id`.** The column has only ever held a target name (the task's `target:`, defined under `targets/`), never the `--env` overlay, which selects configuration values and never reaches the history table.
- This is the **first step needing detection** (`ALTER TABLE` has no portable conditional form), which settled the question deferred in Session 69: **a detection query may now carry positional parameters, every one bound to the current schema** — `Connection.getSchema()`, falling back to `getCatalog()`. That resolves on H2 (`PUBLIC`), PostgreSQL (`public`) and MySQL/MariaDB (no schema; catalog = database name = their `table_schema`), so the generic resource can no longer match a same-named table elsewhere on the server. Rename statement per dialect: MySQL `CHANGE COLUMN` (restating the type; `RENAME COLUMN` needs MySQL 8.0 / MariaDB 10.5.2 and this resource still targets 5.5), PostgreSQL `RENAME COLUMN`, generic add/backfill/drop. Index names keep `_env` — renaming one needs another dialect-divergent step 5.5 cannot express, and nothing reads those names.
- **Operational note**: do not share one history database between migraphe versions spanning this change; older versions query `environment_id`. The API-side `Environment` → `Target` rename is a separate change.
- Tests: +24 (1,109 total). Ordering asserted **in both directions** on H2 and MariaDB 10.1, with a mutation (dropping `id DESC`) confirming both fail on the real server — one direction alone can pass on physical order. Rename verified from a legacy-shaped table with rows preserved on H2, MariaDB 10.1 (`CHANGE COLUMN`) and PostgreSQL, plus a mutation forcing detection true. `RecordIds` covered for lexicographic order, same-millisecond monotonicity, 32 virtual threads × 500 ids, clock regression and UUID validity. See [docs/CHANGELOG.md](docs/CHANGELOG.md).

---

**Last Updated**: 2026-08-20
**Current Work**: Deterministic "latest record" plus the history column rename. `ExecutionRecord` factories mint time-ordered UUIDv7 ids so the existing `id` primary key breaks ties that MariaDB's second-granularity `executed_at` creates (Connector/J truncates fractional seconds because the server reports itself as 5.5.5); ordering is `executed_at DESC, id DESC`, keeping legacy rows in their previous order. `environment_id` became `target_id` via the first detection-guarded migration step, which introduced positional parameters in detection queries bound to the current schema. See [docs/CHANGELOG.md](docs/CHANGELOG.md).
