...
29. **Fingerprint Covers the Whole Definition; Core Owns the Closure (Session 75)**: the token is the SHA-256 of **`up:` SQL + `down:` SQL + `autocommit` + transitive dependencies**, not the UP SQL alone. `down:` is in it because migraphe **cannot read the database** and so must never decide which side of a mismatch is authoritative (decision 28); a token meaning "the object is stale" would be making that call. The set flagged is the same either way — splitting would only say *which field* differs, which is information, not a decision. Named cost: a `down:`-only edit moves the token although no object changed. `MigrationNode.fingerprint()` → **`fingerprint(List<NodeId>)`** (not a break — the method postdates v0.6.0): a node knows only its *direct* dependencies, so **core computes and canonically orders the closure** (`MigrationGraph.canonicalTransitiveDependencies`, ascending `NodeId.value()`) and the plugin folds in what it is handed. The closure walks **declarations**, not the adjacency list, because `fromNodesUp` drops ids outside the supplied list — reading it there made deleting a task file shrink every dependent's closure and report untouched nodes as `[!]`. Residual, pinned by test and accepted: a deleted node's *own* dependencies drop out unless reachable by another declared path. Pre-image framing is `<len>:<text>` per part, `t`/`f` for autocommit, `-` for an absent `down:` — so `[ab]` ≠ `[a, b]` and "rollback deleted" ≠ "rollback blanked". **No scheme prefix** (rejected: a plugin changing its derivation is exactly who would ignore the convention; `amend` is the remedy). See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) decision 34.
...
- Widened the fingerprint from the UP SQL alone to **`up:` SQL + `down:` SQL + `autocommit` + transitive dependencies** — what the history actually records about a node. Including `down:` was first argued *against* and then reversed: keeping it out would have let `[!]` mean "the database is stale", which is exactly the judgement a tool that cannot read the database must never make. The set of nodes flagged is identical either way; a split would only name *which field* differs, which is information, not a decision.
- `MigrationNode.fingerprint()` → **`fingerprint(List<NodeId>)`**. A node knows only its direct dependencies, so **core computes and canonically orders the closure** and the plugin folds in what it is handed — keeping the normalization in one place, since one plugin ordering differently makes every stored token read as changed. Not a break: the method postdates v0.6.0.
- The closure walks **declarations**, not the adjacency list. `fromNodesUp` drops ids outside the supplied node list, so reading it there meant deleting one task file shrank every dependent's closure and reported untouched nodes as `[!]` — in the orphan scenario the feature exists for. One residual is pinned by test and accepted: a deleted node's own dependencies drop out unless another declared path reaches them.
- Framing is `<len>:<text>` per part, `t`/`f` for autocommit, `-` for an absent `down:`, so `[ab]` ≠ `[a, b]` and "rollback deleted" ≠ "rollback blanked". **No derivation-scheme prefix** — rejected because the convention depends on the goodwill of the very plugin whose derivation change it guards against, and `amend` already resolves the mass `[!]` that follows one.
- **The wiring had no test at all.** Every core test double ignored its argument, so replacing the closure with `List.of()` at any of the three call sites failed nothing. Added a double that returns what it is handed and one mutation-checked test per call site; every fixture puts a node two hops away so "direct dependencies" is caught too. Every expected digest — including the two an end-to-end test reads back out of a real PostgreSQL — was computed from the framing before the implementation changed.
- The javadoc was rejected in review for the **fourth** time in this class, again for absolutes with carve-outs, and `downSql.strip()` turned out to be unpinned (the third unpinned mutation this method has had). Both repaired. The `[!]` advice in both user guides was **destructive as written** for a `down:`-only edit and is corrected.
...
**Current Work**: the fingerprint's content is settled — `up:` SQL + `down:` SQL + `autocommit` + the transitive dependencies core computes and canonically orders. `status` reports it as five drift states and `migraphe amend` / `migrapheAmend` resolves it in the history's favour. Next: `rebuild` (roll back and re-apply what drifted, and drop what only exists in the database — needs a `dependencies` column, which would also close the one accepted closure gap), `baseline <id>` (adopt an existing database by inserting rows for unrecorded nodes), and drift warnings in `validate`. **Still open from Session 74**: `DagExecutor` counts a node skipped-while-running twice, reachable with `execution.parallel: true`; it needs an atomic claim and its own change. See [docs/CHANGELOG.md](docs/CHANGELOG.md).
