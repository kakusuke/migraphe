...
**Current Work**: JitPack をエンドユーザー向け配布チャネルとして公式採用。`README*.md` / `docs/USER_GUIDE*.md` / `docs/PLUGIN_DEVELOPMENT*.md` / `sample/cli/` / `sample/gradle/` を `com.github.kakusuke.migraphe:*:v0.2.1` (安定 Git タグ) + JitPack リポジトリ + Gradle `resolutionStrategy.eachPlugin` 形式に書き換え。`main-SNAPSHOT` は `LockSyncChecker` の SNAPSHOT 未対応バグを避けるため意図的に user-facing docs では使わない方針（[CONTRIBUTING.md](CONTRIBUTING.md) の Tag vs main-SNAPSHOT 節で記述）。`sample/cli/migraphe.lock.yaml` は削除し `migraphe pin` を README に追加。`CONTRIBUTING.md` の JitPack セクションは運用 Tips のみに縮小。Phase 22 のスコープは "contributor-only beta" から "primary distribution until Maven Central" に再定義された。
