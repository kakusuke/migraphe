...
plugins:
  - coordinate: com.github.kakusuke.migraphe:migraphe-plugin-mysql:v0.7.0
    repository: jitpack
```
...
- **ルーチン (Routines)** — ストアドプロシージャと関数（定義者の帰属、引数表（位置 / モード / 名前 / 型）、および SQL コードブロックとしての定義本体を含む）。接続アカウントに `ROUTINE_DEFINITION` を読む権限がない場合、本体は出力されません
