...
public final class LockFileBuilder {
    ...
    public LockFile build(List<ResolvedPluginGroup> groups) {
        ...
        for (ResolvedPluginGroup group : groups) {
            String repositoryId = group.plugin().repositoryRef().orElse(DEFAULT_REPOSITORY_ID);
            ...
            plugins.add(new LockedPlugin(group.root().coordinate(), repositoryId, rootSha, deps));
        }
