...
class GraphCanvasTest {
    ...
    @Test
    @DisplayName("buildGrid() は a→b の直線チェーンで row[0][0] が TaskCell(a)、row[2][0] が TaskCell(b) を返す")
    void buildGridReturnsCellsWithCorrectNodeIds() {
        MigrationNode nodeA = TestHelpers.node("a").name("A").build();
        MigrationNode nodeB = TestHelpers.node("b").name("B").dependencies(NodeId.of("a")).build();
        ...
        List<List<Cell>> grid = canvas.buildGrid();
        assertThat(grid.get(0).get(0)).isInstanceOf(Cell.TaskCell.class);
        assertThat(((Cell.TaskCell) grid.get(0).get(0)).id()).isEqualTo(NodeId.of("a"));
        assertThat(grid.get(2).get(0)).isInstanceOf(Cell.TaskCell.class);
        assertThat(((Cell.TaskCell) grid.get(2).get(0)).id()).isEqualTo(NodeId.of("b"));
    }

    @Test
    @DisplayName("buildGrid() は a→b, a→c のブランチ構造でブランチ行の col 0 が ForkCell を返す")
    void buildGridReturnsForkCellForBranchRow() {
        MigrationNode nodeA = TestHelpers.node("a").name("A").build();
        MigrationNode nodeB = TestHelpers.node("b").name("B").dependencies(NodeId.of("a")).build();
                        ...
                        .filter(
                                row ->
                                        row.stream()
                                                .anyMatch(
                                                        cell ->
                                                                cell instanceof Cell.TaskCell tc
                                                                        && tc.id()
                                                                                .equals(
                                                                                        NodeId.of(
                                                                                                "b"))))
        ...
        assertThat(branchRow.get(0)).isInstanceOf(Cell.ForkCell.class);
    }

    @Test
    @DisplayName("buildGrid() は非支配木辺のソース行のレーン列に LaneStartCell を配置する")
    void buildGridPlacesLaneStartCellAtNonDomEdgeSourceRow() {
        MigrationNode nodeA = TestHelpers.node("a").build();
        MigrationNode nodeB = TestHelpers.node("b").dependencies(NodeId.of("a")).build();
        MigrationNode nodeC = TestHelpers.node("c").dependencies(NodeId.of("a")).build();
        MigrationNode nodeD =
        ...
        // b は非支配木辺グループの最初のソース（トポロジー順で先） → LaneStartCell が置かれる
        List<Cell> bRow =
                grid.stream()
                        .filter(
                                row ->
        ...
        boolean hasLaneStart = bRow.stream().anyMatch(cell -> cell instanceof Cell.LaneStartCell);
        assertThat(hasLaneStart).isTrue();
    }

    @Test
    @DisplayName("buildGrid() はダイヤモンド構造のターゲット行直前に LaneCloseCell を含むマージ行を挿入する")
    void buildGridInsertsMergeRowWithLaneCloseCellForDiamondTarget() {
        MigrationNode nodeA = TestHelpers.node("a").build();
        MigrationNode nodeB = TestHelpers.node("b").dependencies(NodeId.of("a")).build();
        MigrationNode nodeC = TestHelpers.node("c").dependencies(NodeId.of("a")).build();
        MigrationNode nodeD =
                                                ...
                                                .anyMatch(
                                                        cell ->
                                                                cell
                                                                        instanceof
                                                                        Cell.LaneCloseCell));
        assertThat(hasLaneCloseRow).isTrue();
    }

    @Test
    @DisplayName("render() は post-trunk の最後の branch でも └ でなく ├ を使う（├ に統一）")
    void renderUsesForkInsteadOfElbowForLastBranch() {
        // a→b, a→c, b→f, b→g, f+g→h, h+c→e の構造で
        // e は post-trunk branch（isLastChild=true） → 旧実装は "└─●" / 新実装は "├─●"
        MigrationNode nodeA = TestHelpers.node("a").build();
        MigrationNode nodeB = TestHelpers.node("b").dependencies(NodeId.of("a")).build();
        MigrationNode nodeC = TestHelpers.node("c").dependencies(NodeId.of("a")).build();
        MigrationNode nodeF = TestHelpers.node("f").dependencies(NodeId.of("b")).build();
        MigrationNode nodeG = TestHelpers.node("g").dependencies(NodeId.of("b")).build();
        MigrationNode nodeH =
                TestHelpers.node("h").dependencies(NodeId.of("f"), NodeId.of("g")).build();
        MigrationNode nodeE =
                TestHelpers.node("e").dependencies(NodeId.of("h"), NodeId.of("c")).build();

        DominatorTree dt =
                new DominatorTree(List.of(nodeA, nodeB, nodeC, nodeF, nodeG, nodeH, nodeE), false);
        GraphCanvas canvas = new GraphCanvas();
        canvas.layout(dt);

        List<String> lines = canvas.render(n -> n.id().value());

        // render() は └ を一切使わない（└ は廃止、├ に統一）
