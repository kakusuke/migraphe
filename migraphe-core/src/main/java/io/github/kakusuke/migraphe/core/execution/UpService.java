...
public final class UpService {
     ...
     * @return the plan
     */
    public UpPlan plan(@Nullable NodeId requestedNode) {
        return plan(requestedNode, true);
    }

    /**
     * The same plan, without refusing on migrations that were edited since they ran.
     *
     * <p>For {@code rebuild}, and only for it. Refusing to apply onto a database that does not
     * match the definitions is right for {@code up}, which would be building on ground the task
     * files no longer describe — and wrong for the command whose entire job is to remove that
     * mismatch, which would otherwise be stopped by the state it exists to repair.
     *
     * @param requestedNode the highest node to migrate up to and including, or {@code null} to
     *     consider every node
     * @return the plan
     */
    public UpPlan planWhileRepairingDrift(@Nullable NodeId requestedNode) {
        return plan(requestedNode, false);
    }

    private UpPlan plan(@Nullable NodeId requestedNode, boolean refuseOnDrift) {
        ...
        Set<NodeId> unrecordable = graph.nodesWhoseRollbackWouldNotBeRecorded();
        if (!unrecordable.isEmpty()) {
            return new UpPlan(new UpBlocker.UnrecordableRollback(unrecordable), Set.of());
        }

        UnusableAppliedRows unusable = UnusableAppliedRows.of(graph, historyRepository);
        Set<NodeId> unreadable = new LinkedHashSet<>(unusable.unreadable());
        unreadable.addAll(
                UnusableAppliedRows.nodesThatCannotReportTheirContent(
                        graph, selectedNodes(requestedNode)));
        if (!unreadable.isEmpty()) {
            return new UpPlan(new UpBlocker.UnreadableContent(unreadable), Set.of());
        }
        if (!unusable.incomplete().isEmpty()) {
            return new UpPlan(new UpBlocker.IncompleteHistory(unusable.incomplete()), Set.of());
        }
        if (refuseOnDrift && !unusable.edited().isEmpty()) {
            return new UpPlan(new UpBlocker.EditedSinceApplied(unusable.edited()), Set.of());
        }

        return new UpPlan(null, selectedNodes(requestedNode));
    }

    /**
     * Returns the nodes an UP run would execute, ignoring anything that would refuse the run.
     *
     * <p>When {@code requestedNode} is supplied, the candidate set is that node plus all of its
     ...
     */
    public Set<NodeId> selectedNodes(@Nullable NodeId requestedNode) {
        Set<NodeId> candidates;

        if (requestedNode != null) {
            candidates = new HashSet<>(graph.getAllDependencies(requestedNode));
            candidates.add(requestedNode);
        } else {
            candidates =
        ...
        return candidates.stream()
                .filter(
                        id -> {
                            ...
                            return !historyRepository.wasExecuted(id);
                        })
     ...
     * @param selectedNodes the nodes a run would execute; always empty when {@code blocker} is
     *     non-{@code null}
     */
    public record UpPlan(@Nullable UpBlocker blocker, Set<NodeId> selectedNodes) {}
}
