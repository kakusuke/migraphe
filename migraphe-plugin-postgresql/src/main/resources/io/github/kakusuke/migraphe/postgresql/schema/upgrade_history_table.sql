-- History-table upgrades for the PostgreSQL plugin, in order.
--
...
-- detection query that skips it once it is in place.
--
...
-- bare placeholder and initializes cleanly on PostgreSQL 16. It is kept because it costs nothing
-- and states the comparison's type here rather than leaving it to how the driver happens to bind
-- the parameter.

-- Renames the column that older versions called environment_id. The name always held a target id
-- (targets/*.yaml), never a deployment environment. The indexes covering it keep their names and
-- follow the column automatically, so they need no step of their own here.
...
-- Creates the indexes the creation resource declined to make while the column was still called
-- environment_id. A history an older release created already has them under these names, and the
-- rename above carried them onto the new column, so the guard finds them and this is a no-op there.
-- It is not a no-op for a table that reached the legacy shape without them.
--@check node/target index
SELECT 1 FROM pg_indexes
 WHERE schemaname = CAST(? AS text)
   AND indexname = 'idx_migraphe_history_node_env';
--@apply
